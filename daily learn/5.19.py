...
#     return res
#
# def pinjie(list1,list2):##将切割数字与符号进行分割
...
#
#
# def panduan(s,target):
#     res=[]
#     for i in qiege(s):
#         len1=len(i)
#         for j in zuhe(['+','*','-'],len1-1):
#             if int(i[0])>target and '-' not in j and '0' not in i:
#                 continue
#             if eval(pinjie(i,j))==target:
#                 res.append(pinjie(i,j))
#     return res
# print(panduan("9999999999",1409865407))
...
#     else:
#         boards = [0]*k
#         def dfs(matchsticks, boards, board, ind):
#             if ind == len(matchsticks):
#                 return True
#             for i in range(len(boards)):
#                 if boards[i] + matchsticks[ind] <= board:
#                     boards[i] += matchsticks[ind]
#                     if dfs(matchsticks, boards, board, ind + 1):
#                         return True
#                     boards[i] -= matchsticks[ind]
#             return False
#         return dfs(matchsticks, boards, board, 0)
...
#         res=max(res,root.val,root.left.val)
#         bt(root.left)
#     elif not root.left and root.right:
#         root.val+=root.right.val
#         res = max(res, root.val,root.right.val)
#         bt(root.right)
#     else:
#         root.val+=(root.right.val+root.left.val)
#         res = max(res, root.val,bt(root.left),bt(root.right))
#     return res
# print(bt(a))
# def sumNumbers1(root):
#     res=[]
...
#         if not root:
#             return
#         if not root.left and not root.right:######保证是结点
#                 res.append(path+[str(root.val)])
#         bt(root.left,path+[str(root.val)])
#         bt(root.right,path+[str(root.val)])
#     bt(root,[])
...
def triangleNumber(nums):
    ...
    def bt(path, start):
        ...
        if len(path)>3:
            return
        for i in range(start, len(nums)):
            if len(path)==2 and sum(path)<=nums[i]:
                continue
            else:
                path.append(nums[i])
                bt( path,i+1)
