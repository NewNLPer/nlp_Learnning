...
train_data_path=r"C:\Users\NewNLPer\Desktop\za\竞赛数据集\情感分类数据\pre_traindata.csv"
dev_data_path=r"C:\Users\NewNLPer\Desktop\za\竞赛数据集\情感分类数据\pre_devdata.csv"
predict_data_path=r"C:\Users\NewNLPer\Desktop\za\竞赛数据集\情感分类数据\pre_testdata.csv"
...
predict_dataloader=Data.DataLoader(
        ...
        batch_size=batch_size,
        collate_fn=collate_fn_train_dev)###文本与标签不应该被打乱
...
def train_dev_predict(train_dataloader,dev_dataloader,predict_dataloader,epoch_times,optimize,model,loss_fn,save_path):
    ...
    for epoch in range(1,epoch_times+1):
        sum_loss=0
        iteration_nums=0
        for input_ids,attention_mask,tokens_type_ids,label in train_dataloader:
        ...
        with torch.no_grad():
            pre_label_list=[]
            truth_label_list=[]
            for input_ids,attention_mask,tokens_type_ids,label in dev_dataloader:
                input_ids=input_ids.to(device)
                attention_mask=attention_mask.to(device)
                tokens_type_ids=tokens_type_ids.to(device)
...
if __name__=='__main__':
    save_path=r"C:\Users\NewNLPer\Desktop\za\竞赛数据集\情感分类数据\submit_example.csv"
