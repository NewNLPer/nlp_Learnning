...
def single_plot(b,k_1,k_2):
    ...
    if k_1 == -1: # k_1 [0,1]
        result_finally = []
        line_space_k = linespace(0, 1, 0.05)
        for k in tqdm(line_space_k):
            result = odeint(Cooperation_proportion_derivatives, initial_x, t, args=(b, alph, yit, sit,k,k_2))
            result_finally.append(get_round(result[-1].tolist()))
        return [line_space_k,result_finally]

    if k_2 == -1: # k_2 [0,1]
        result_finally = []
        line_space_k = linespace(0, 1, 0.05)
        for k in tqdm(line_space_k):
            result = odeint(Cooperation_proportion_derivatives, initial_x, t, args=(b, alph, yit, sit,k_1,k))
            result_finally.append(get_round(result[-1].tolist()))
        return [line_space_k,result_finally]
...
def plt_b_ks_figure(b):
    line_space_k = linespace(0, 1, 0.05)
    ########### K_1 ######################
    for k_1 in [0,0.3,0.7,1]:
        result = single_plot(b,k_1, -1)
        pc = [item[0] for item in result[-1]]
        plt.xlabel('$k_{2}$')
        plt.ylabel(r'$\rho_c$')
        plt.plot(line_space_k, pc, label="$k_{1}=%s$" % (k_1), marker='*')
    plt.legend()
    plt.title('$b=%s$'%(b))
    plt.show()
...
################## K_2 #####################
    ...
    for k_2 in [0,0.3,0.7,1]:
        result = single_plot(b,-1, k_2)
        pc = [item[0] for item in result[-1]]
        plt.xlabel('$k_{1}$')
        plt.ylabel(r'$\rho_c$')
        plt.plot(line_space_k, pc, label="$k_{2}=%s$" % (k_2), marker='*')
    plt.legend()
    plt.title('$b=%s$'%(b))
    plt.show()
